...
> 新增页面模板「**左树右表列表页**」（页面模板 8 → **9**）：覆盖**主子表结构**的列表场景——左树选中主表主数据、右表展示其子表数据，主子各自增删改查。

### Added

- **页面模板「左树右表列表页」**（[`page-tree-table.md`](skills/pangea-design-vue/references/patterns/page-tree-table.md) + 脚手架 `src/pages/TreeTable/index.vue`，预览路由 `/tree-table`），参照 Figma `219:4241` 实现：页头（标题 + 帮助文档）+ 左树面板 **260px**（`+` 新增下拉 + 搜索 + `a-tree block-node`）+ 右侧子表区（操作栏 + 表格 + 分页）；左右**各自内部滚动**，页面不整体滚。
  - **写进模板的三条联动契约**（左树右表最容易做错的地方）：① 树选中项是右侧数据的**唯一来源**，切主数据要**重拉 + 分页复位 + 清空已勾选行**（不复位会出现「切到只有 8 条的主数据却停在第 3 页 → 右侧空白」，不清空勾选会把上一条主数据的选中带过来导致误批量操作）；② **首屏不预选任何节点，且未选主数据时不摆空表格**，给 `a-empty`（图标 + 「先从左侧列表选择」）引导，否则用户分不清是没数据还是没选；③ 子表「创建」必须依附当前主数据，避免产生挂空的子记录。
  - **选型判断写进决策树**：新建子数据必须指定父级（**归属**）→ 用本模板；层级只是**筛选维度** → 简单列表页加个筛选项就够，不要上主子表。
  - 树搜索为前端过滤且**保留命中节点的父链**（只留命中节点会让层级断裂）；删除主数据的确认文案**明确提示会级联删除子数据**。
  - **新增入口收敛到左上角下拉**（「新增根级」+「新增子级」，后者未选中节点时禁用）；节点级只留「编辑 / 删除」，用 `#extra` 插槽 + `arco-icon-hover` + **hover 才显形**（常驻会让树很吵），图标需 `@click.stop` 否则点操作会顺带选中节点。
  - 附带三个 Arco 实测坑（已写进模板文档与注释）：`#extra` 被渲染在 `.arco-tree-node-title` **之后**、而高亮底色加在 title 上，图标必须**绝对定位**才落进高亮区（且不必给 title 留 `padding`，盖住文字即可）；纯图标按钮虽自动获得 `.arco-btn-only-icon`（28×28），但作为 flex 子项会被撑满的搜索框**压扁**，需 `flex-shrink: 0`（`shape="square"` 在本主题无对应 CSS，加了无效）；`arco-icon-hover` 的圆底是绝对定位 `::before`，**图标包图标不是 `.arco-icon`** 吃不到 Arco 的垫高规则，会被圆底盖住，需自己给 svg 加 `position: relative`。
  - 右侧子表形态直接沿用[简单列表页](skills/pangea-design-vue/references/patterns/page-simple-list.md)（操作栏 / 表格 / 分页规格一致），不另造一套。
  - 设计稿只画了左上角的 `+`，**节点级增删改属增补**（需求明确要求主子表各自增删改查），已在文档标注。
- `SKILL.md` 决策树新增分支与索引表新增一行；`catalog.json` 页面模板 **8 → 9**。

### Verified

- 脚手架 + website 双 `npm run gate` 通过；SKILL.md 内部链接可达性自检通过。
- 联动契约经 Playwright 实测：未选主数据时右侧为空状态引导且无表格 → 选中「上海分公司」后出表格（数据随主数据变化）→ **翻到第 2 页并勾选 1 行后切换到「北京分公司」，分页复位为 1、勾选归 0、数据正确切换**；树搜索「杭州」保留父链（华东大区 + 杭州分公司）；节点 hover 出「更多」且删除弹出含级联提示的确认框；子表行「更多」为 编辑/删除。无 console/page error。
- 交互细节经 Playwright 实测：首屏 selected 数为 **0** 且占位含 48px 图标 + 「先从左侧列表选择」、不渲染表格；`+` 按钮 **28×28**（`arco-btn-only-icon` + `flex-shrink:0`）；新增下拉两项，未选中时「新增子级」带 `arco-dropdown-option-disabled`、选中后解除；节点「更多」带 `arco-icon-hover` 且其矩形**完整落在 title 高亮矩形内**，hover 时圆底 `#f2f3f5` + 图标主色 `rgb(0,170,166)` 清晰可辨（svg `position: relative` 生效）；节点菜单仅 编辑/删除。
