...
const menuItems = ref<MenuItem[]>([
  { key: '/', title: '介绍' },
  { key: '/guide', title: '使用指南' },
  { key: '/foundations', title: '设计基础' },
  { key: '/components', title: '组件预览' },
  { key: '/templates', title: '页面模板' },
  { key: '/changelog', title: '更新日志' },
]);

// 菜单选中（子路由如 /templates/xxx 高亮其父项 /templates）
const selectedKeys = computed(() => {
  const p = route.path;
  if (p === '/') return ['/'];
  const item = menuItems.value.find(
    (m) => m.key !== '/' && (p === m.key || p.startsWith(m.key + '/'))
  );
  return [item ? item.key : p];
});
    ...
    <div class="pg-layout__body">
      ...
      >
        ...
        <div v-show="!collapsed" class="pg-layout__sidebar-head">
          <span class="pg-layout__sidebar-title">设计系统</span>
        </div>
