...
- **页面模板「左树右表列表页」**（[`page-tree-table.md`](skills/pangea-design-vue/references/patterns/page-tree-table.md) + 脚手架 `src/pages/TreeTable/index.vue`，预览路由 `/tree-table`），参照 Figma `219:4241` 实现：页头（标题 + 帮助文档）+ 左树面板 **260px**（`+` 新增下拉 + 搜索 + `a-tree block-node`）+ 右侧子表区（操作栏 + 表格 + 分页）；左右**各自内部滚动**，页面不整体滚。
  ...
  - 附带两个 Arco 实测坑（已写进模板文档与注释）：`#extra` 被渲染在 `.arco-tree-node-title` **之后**、而高亮底色加在 title 上，图标必须**绝对定位**才落进高亮区；纯图标按钮虽自动获得 `.arco-btn-only-icon`（28×28），但作为 flex 子项会被撑满的搜索框**压扁**，需 `flex-shrink: 0`（`shape="square"` 在本主题无对应 CSS，加了无效）。
...
- 交互细节经 Playwright 实测：首屏 selected 数为 **0** 且占位含 48px 图标 + 「先从左侧列表选择」、不渲染表格；`+` 按钮 **28×28**（`arco-btn-only-icon` + `flex-shrink:0`）；新增下拉两项，未选中时「新增子级」带 `arco-dropdown-option-disabled`、选中后解除；节点「更多」带 `arco-icon-hover` 且其矩形**完整落在 title 高亮矩形内**；节点菜单仅 编辑/删除。
